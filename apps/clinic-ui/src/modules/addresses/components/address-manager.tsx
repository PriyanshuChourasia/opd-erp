...
export function AddressManager({ addressableType, addressableId }: AddressManagerProps) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey }); closeSheet(); toast.success("Address added successfully"); },
    onError: (err) => { toast.error(extractApiError(err)); },
  });
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateAddressInput }) => updateAddress(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey }); closeSheet(); toast.success("Address updated successfully"); },
    onError: (err) => { toast.error(extractApiError(err)); },
  });
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="sm:max-w-md overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>{editingId ? "Update address details." : "Add a new address."}</SheetDescription>
          </SheetHeader>
          ...
          <SheetFooter>
            ...
            <Button onClick={handleSave} disabled={!(form as CreateAddressInput).addressLine1?.trim() || isPending}>
